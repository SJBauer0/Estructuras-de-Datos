...
T List<T>::get(int index) const {
	...
	return p->value; // return the value of the node
}
...
// =================================================================
template <class T>
void List<T>::insert_at(T val, int index) {
    Node<T> *p, *q; // p is the node before the insertion point
					// q is the node to be inserted
	...
	if (index == size) { // insert at the end
		push_back(val); // call the push_back method
		return;
	}

	if (index == 0) { // insert at the beginning
		push_front(val); // call the push_front method
		return;
	}

	p = head; // p will be the node before the insertion
	for (int i = 0; i < index - 1; i++) { // find the node before the insertion
		p = p->next; // p is the node before the insertion
	}

	q = new Node<T>(val); // create the node to be inserted

	q->next = p->next; // insert q after p
	p->next = q;
